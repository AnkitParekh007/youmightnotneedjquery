...
export class RolesService {
	...
	constructor(@Inject(USE_ROLES_STORE) private isolate: boolean = false, private rolesStore: RolesStore, private permissionsService: PermissionsService) {
	...
	public hasOnlyRoles(names: string | string[]): Promise<boolean> {
		...
		return Promise.all([this.hasRoleKey(names), this.hasRolePermission(this.rolesSource.value, names)]).then(([hasRoles, hasPermissions]: [boolean, boolean]) => {
	...
	private hasRoleKey(roleName: string[]): Promise<boolean> {
		const promises: Observable<boolean>[] = roleName.map((key) => {
			const hasValidationFunction =
				!!this.rolesSource.value[key] && !!this.rolesSource.value[key].validationFunction && isFunction(this.rolesSource.value[key].validationFunction);
	...
	private hasRolePermission(roles: RolesObject, roleNames: string[]): Promise<boolean> {
