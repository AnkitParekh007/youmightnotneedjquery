...
import {
	...
	RedirectToNavigationParameters,
	OnlyFn,
	RedirectTo,
	RedirectToFn
...
export class PermissionsGuard implements CanActivate, CanLoad, CanActivateChild {
	...
	private transformPermission(permissions: PermissionsRouterData, route: ActivatedRouteSnapshot | Route, state?: RouterStateSnapshot): PermissionsData {
		const only = isFunction<OnlyFn>(permissions.only) ? permissions.only(route, state) : transformStringToArray(permissions.only);
		const except = isFunction<ExceptFn>(permissions.except) ? permissions.except(route, state) : transformStringToArray(permissions.except);
	...
	private passingExceptPermissionsValidation(permissions: PermissionsData, route: ActivatedRouteSnapshot | Route, state: RouterStateSnapshot): Promise<boolean> {
		if (
			!!permissions.redirectTo &&
			(isFunction<RedirectToFn>(permissions.redirectTo) || (isPlainObject(permissions.redirectTo) && !this.isRedirectionWithParameters(permissions.redirectTo)))
		) {
			...
			return from(permissions.except)
				.pipe(
					mergeMap((permissionsExcept) => {
						return forkJoin([this.permissionsService.hasPermission(permissionsExcept), this.rolesService.hasOnlyRoles(permissionsExcept)]).pipe(
							tap((hasPermissions) => {
	...
	private redirectToAnotherRoute(
		...
		const redirectTo = isFunction<RedirectToFn>(permissionRedirectTo) ? permissionRedirectTo(failedPermissionName, route, state) : permissionRedirectTo;
	...
	private isRedirectionWithParameters(object: any | RedirectToNavigationParameters): object is RedirectToNavigationParameters {
		return isPlainObject(object) && (!!object.navigationCommands || !!object.navigationExtras);
	}

	private transformNavigationCommands(navigationCommands: any[] | NavigationCommandsFn, route: ActivatedRouteSnapshot | Route, state?: RouterStateSnapshot): any[] {
	...
	private onlyRedirectCheck(permissions: PermissionsData, route: ActivatedRouteSnapshot | Route, state?: RouterStateSnapshot): Promise<boolean> {
		let failedPermission = "";

		return from(permissions.only)
			.pipe(
				mergeMap((permissionsOnly) => {
					return forkJoin([this.permissionsService.hasPermission(permissionsOnly), this.rolesService.hasOnlyRoles(permissionsOnly)]).pipe(
						tap((hasPermissions) => {
	...
	private handleRedirectOfFailedPermission(permissions: PermissionsData, failedPermission: string, route: ActivatedRouteSnapshot | Route, state?: RouterStateSnapshot) {
	...
	private isFailedPermissionPropertyOfRedirectTo(permissions: PermissionsData, failedPermission: string): boolean {
		return !!permissions.redirectTo && permissions.redirectTo[failedPermission];
	}

	private checkOnlyPermissions(purePermissions: PermissionsData, route: ActivatedRouteSnapshot | Route, state?: RouterStateSnapshot): Promise<boolean> {
	...
	private passingOnlyPermissionsValidation(permissions: PermissionsData, route: ActivatedRouteSnapshot | Route, state?: RouterStateSnapshot): Promise<boolean> {
		if (isFunction<RedirectToFn>(permissions.redirectTo) || (isPlainObject(permissions.redirectTo) && !this.isRedirectionWithParameters(permissions.redirectTo))) {
			return this.onlyRedirectCheck(permissions, route, state);
		}
		return this.checkOnlyPermissions(permissions, route, state);
	}
