...
@Component({
	selector: "-permissions-lazy",
	template: "lazy-loaded-parent [<router-outlet></router-outlet>]"
})
class ParentLazyLoadedComponent {}

function getLazyLoadedModule(importedModule: any) {
	@Component({ selector: "-permissions-lazy", template: "lazy-loaded-child" })
...
describe("module", () => {
	beforeEach(() => {
		TestBed.configureTestingModule({
			imports: [RouterTestingModule, PermissionsModule.forRoot()],
			declarations: [RootComponent]
		});
	});

	it("should work when lazy loaded using forChild", fakeAsync(
		inject([Router, Location, NgModuleFactoryLoader], (router: Router, location: Location, loader: SpyNgModuleFactoryLoader) => {
			const LoadedModule = getLazyLoadedModule(PermissionsModule.forChild());
			...
			router.resetConfig([
				{
					path: "lazy",
					loadChildren: () => import("expected").then((m) => m.default)
				}
			]);
			...
			// since the root module imports the PermissionsModule with forRoot and the lazy loaded module with forChild
			// the permissionsService service is shared between both modules
			// the constructor of the ChildLazyLoadedComponent overwrote the "ADMIN" key of the root PermissionsService
			permissionsService.hasPermission("ADMIN").then((data) => {
	...
	it("should work when loaded using just Module", fakeAsync(
		inject([Router, Location, NgModuleFactoryLoader], (router: Router, location: Location, loader: SpyNgModuleFactoryLoader) => {
			const LoadedModule = getLazyLoadedModule(PermissionsModule);
			loader.stubbedModules = { expected: LoadedModule };
	...
	it("should create 2 instances of the service when lazy loaded using forRoot", fakeAsync(
		inject([Router, Location, NgModuleFactoryLoader], (router: Router, location: Location, loader: SpyNgModuleFactoryLoader) => {
			const LoadedModule = getLazyLoadedModule(PermissionsModule.forRoot());
			loader.stubbedModules = { expected: LoadedModule };
			...
			// since both the root module and the lazy loaded module use forRoot to define the PermissionsModule
	...
	it("should create 2 instances of the service when lazy loaded using forChild and isolate true", fakeAsync(
		inject([Router, Location, NgModuleFactoryLoader], (router: Router, location: Location, loader: SpyNgModuleFactoryLoader) => {
			const LoadedModule = getLazyLoadedModule(PermissionsModule.forChild({ permissionsIsolate: true }));
			loader.stubbedModules = { expected: LoadedModule };
			...
			// the constructor of the ChildLazyLoadedComponent didn't overwrote the "false" key of the root PermissionsService
			permissionsService.hasPermission("ADMIN").then((data) => {
				expect(data).toBe(false);
			});
		})
...
@Component({
	selector: "-permissions-root-roles",
	template: ` <router-outlet></router-outlet> `
})
...
function getLazyRolesLoadedModule(importedModule: ModuleWithProviders) {
	@Component({ selector: "-permissions-lazy", template: "lazy-loaded-child" })
	class ChildLazyLoadedComponent {
		constructor(public permissions: RolesService) {
...
describe("Role module", () => {
	...
	it("should work when lazy loaded using forChild", fakeAsync(
		inject([Router, Location, NgModuleFactoryLoader], (router: Router, location: Location, loader: SpyNgModuleFactoryLoader) => {
			const LoadedModule = getLazyRolesLoadedModule(PermissionsModule.forChild());
			loader.stubbedModules = { expected: LoadedModule };

			const fixture = createRoot(router, RootRolesComponent);
			const injector = getTestBed();
			const rolesService: RolesService = injector.get(RolesService);
	...
	it("should create 2 instances of the service when lazy loaded using forRoot", fakeAsync(
		inject([Router, Location, NgModuleFactoryLoader], (router: Router, location: Location, loader: SpyNgModuleFactoryLoader) => {
			const LoadedModule = getLazyRolesLoadedModule(PermissionsModule.forRoot());
			loader.stubbedModules = { expected: LoadedModule };
			...
			// since both the root module and the lazy loaded module use forRoot to define the PermisionsModule
	...
	it("should create 2 instances of the service when lazy loaded using forChild and isolate true", fakeAsync(
		inject([Router, Location, NgModuleFactoryLoader], (router: Router, location: Location, loader: SpyNgModuleFactoryLoader) => {
			const LoadedModule = getLazyRolesLoadedModule(PermissionsModule.forChild({ rolesIsolate: true }));
			loader.stubbedModules = { expected: LoadedModule };
			...
			// since both the root module and the lazy loaded module use forRoot to define the PermissionsModule
			// the permissions service is NOT shared, and 2 instances co-exist
			// the constructor of the ChildLazyLoadedComponent didn't overwrote the "false" key of the root RolesService
			rolesService.hasOnlyRoles("ADMIN").then((data) => {
				expect(data).toBe(false);
			});
		})
