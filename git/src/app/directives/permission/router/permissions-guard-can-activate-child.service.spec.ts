...
describe("Permissions guard only", () => {
	...
	it("should return true when only fulfills", fakeAsync(() => {
		testRoute = {
			data: {
				permissions: {
					only: "ADMIN"
		...
		(permissionGuard.canActivateChild(testRoute, {} as RouterStateSnapshot) as any).then((data) => {
	...
	it("should return true when neither only not except specified", fakeAsync(() => {
		...
		expect(permissionGuard.canActivateChild(testRoute, {} as RouterStateSnapshot)).toBe(true);
	}));
...
describe("Permissions guard test redirectTo path parameters dynamically", () => {
	...
	it("should redirect to parameters specified on navigation commands and navigationExtras", fakeAsync(() => {
		...
		(permissionGuard.canActivateChild(testRoute, {} as RouterStateSnapshot) as any).then((data) => {
			expect(data).toEqual(false);
			expect(fakeRouter.navigate).toHaveBeenCalledWith(["123"], {
				skipLocationChange: true
			});
		});
	}));
...
describe("Permissions guard test redirectTo path multiple redirectionRule", () => {
	...
	it("should redirect dashboard can canRead Agenda fulfills can edit agenda fails", fakeAsync(() => {
		testRoute = {
			data: {
				permissions: {
