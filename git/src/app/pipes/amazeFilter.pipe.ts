...
@Injectable()
export class AmazeFilterPipe implements PipeTransform {
	...
	transform(array: any[], filter: any): any {
		...
		switch (typeof filter) {
			...
			case "string":
				if (AmazeFilterPipe.isNumber(filter)) {
	...
	private filterByObject(filter) {
		return (value) => {
			for (const key in filter) {
				if (key === "$or") {
					if (!this.filterByOr(filter.$or)(AmazeFilterPipe.getValue(value))) {
						return false;
					}
					continue;
				}

				if (!value || !AmazeFilterPipe.isFoundOnWalking(value, key)) {
					return false;
				}

				if (!this.isMatching(filter[key], AmazeFilterPipe.getValue(value[key]))) {
					return false;
				}
			}
			return true;
...
// In HTML template
// {{ collection | amazefilter: searchTerm }}
